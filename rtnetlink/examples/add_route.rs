...
async fn main() -> Result<(), ()> {
    ...
    let dest: IpNetwork = args[1].parse().unwrap_or_else(|_| {
...
async fn add_route(dest: &IpNetwork, gateway: &IpNetwork, handle: Handle) -> Result<(), Error> {
    let route = handle.route();
    route
        .add()
        .dst_prefix(dest.ip(), dest.prefix())
        .gateway(gateway.ip())
        .execute()
        .await?;
    Ok(())
