        ...
        NetlinkPayload::InnerMessage(XfrmMessage::AddPolicy(xm)) => {
            ...
            for attr in xm.nlas.iter() {
                match attr {
                    ...
                    Template(ut) => {
                        assert_eq!(ut.id.daddr.addr[0..4], [198, 51, 100, 20]);
                        assert_eq!(ut.id.daddr.to_ipv4(), Ipv4Addr::from_str("198.51.100.20").unwrap());
                        assert_eq!(ut.id.spi, 0);
                        assert_eq!(ut.id.proto, IPPROTO_ESP);
                        assert_eq!(ut.family, AF_INET);
                        assert_eq!(ut.saddr.addr[0..4], [192, 0, 2, 1]);
                        assert_eq!(ut.saddr.to_ipv4(), Ipv4Addr::from_str("192.0.2.1").unwrap());
                        assert_eq!(ut.reqid, 1);
                        assert_eq!(ut.mode, XFRM_MODE_TUNNEL);
                        assert_eq!(ut.share, XFRM_SHARE_ANY);
                        assert_eq!(ut.optional, 0);
                        assert_eq!(ut.aalgos, 0xFFFFFFFF);
                        assert_eq!(ut.ealgos, 0xFFFFFFFF);
                        assert_eq!(ut.calgos, 0xFFFFFFFF);
                    },
                    Mark(mark) => {
        ...
        NetlinkPayload::InnerMessage(XfrmMessage::AddPolicy(xm)) => {
            ...
            for attr in xm.nlas.iter() {
                match attr {
                    PolicyType(up) => assert_eq!(up.ptype, XFRM_POLICY_TYPE_SUB),
                    Template(ut) => {
                        assert_eq!(ut.id.daddr.to_ipv6(), Ipv6Addr::from_str("fe68::198.51.100.20").unwrap());
                        assert_eq!(ut.id.spi, 0);
                        assert_eq!(ut.id.proto, IPPROTO_ESP);
                        assert_eq!(ut.family, AF_INET6);
                        assert_eq!(ut.saddr.to_ipv6(), Ipv6Addr::from_str("fe68::192.0.2.1").unwrap());
                        assert_eq!(ut.reqid, 1);
                        assert_eq!(ut.mode, XFRM_MODE_TUNNEL);
                        assert_eq!(ut.share, XFRM_SHARE_ANY);
                        assert_eq!(ut.optional, 0);
